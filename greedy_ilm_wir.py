...
class GreedyILMWIR(SearchMethod):
    ...
    def __init__(self, wir_method="unk", max_collapse_width=3, debug=False):
        self.wir_method = wir_method
        self.max_collapse_width = max_collapse_width
        self.debug = debug
    ...
    def _collapse_indices(self, indices):
        l, r = 2048, -2048
        collapsed = []
        last = 0

        for i, idx in enumerate(indices):
            if i - last == self.max_collapse_width or idx < l - 1 or idx > r + 1:
                if l != 2048:
                    collapsed.append(sorted(indices[last:i]))
    ...
    def _perform_search(self, initial_result):
        ...
        ordered_indices, search_over = self._get_index_order(attacked_text)
        index_order = self._collapse_indices(ordered_indices)
        ...
        if self.debug:
            print(len(cur_result.attacked_text.words))
            print(cur_result.attacked_text.words)
            print(cur_result.attacked_text.attack_attrs['original_index_map'])

        while i < len(index_order) and not search_over:
            cur_idx = index_order[i][0]

            transformed_text_candidates = self.get_transformations(
                cur_result.attacked_text,
                original_text=initial_result.attacked_text,
                indices_to_modify=[*range(cur_idx, cur_idx + len(index_order[i]))],
            )
            i += 1
            ...
            if cur_result.goal_status == GoalFunctionResultStatus.SUCCEEDED:
                ...
                if self.debug:
                    print(len(best_result.attacked_text.words))
                    print(best_result.attacked_text.words)
                    print(best_result.attacked_text.attack_attrs['original_index_map'])
